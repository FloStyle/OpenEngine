...
//!
//! Drives the guest export `openengine_gameplay_tick`, which runs the pure
//! `gameplay_tick` in `openengine-logic-sandbox`:
//! 1. serialize `[frame u64 LE (8)][GameplayInputWire (16)][postcard columns]
//!    [Transform | Velocity3D | Actor arena]`,
//! 2. write it into a guest-allocated input buffer (`openengine_alloc`),
//! 3. call `openengine_gameplay_tick(input, len, out, cap)`,
//! 4. read + decode the returned `WorldDelta`.
//!
//! Player input and the deterministic frame counter are pure DATA (Phase D);
//! the host never writes velocities or actor state directly.
...
/// Host driver for the guest gameplay module (`openengine_gameplay_tick`).
...
impl WasmGameplayHost {
    /// Load a logic module exposing `openengine_alloc` +
    /// `openengine_gameplay_tick`.
    ...
    /// Run one guest gameplay tick at `frame` over `world`; apply nothing here,
    /// return the `WorldDelta` so the caller applies it to its PLAY world.
    pub fn tick(&mut self, world: &World, frame: u64) -> anyhow::Result<WorldDelta> {
        ...
        // Fixed header (8-byte frame + 16-byte input wire) then columns+arena.
