...
| `crates/harness` | Headless live-state surface for agents/AI: HTTP server (`/observe` `/spawn` `/set` `/tick` `/hash` `/load_wasm` `/prove` `/transaction` `/save` `/load` `/snapshot` `/restore` `/verify` `/reload_logic` `/schema` `/ai/status` `/ask`, + `/frame` under the `capture` feature) + `openengine-runner` (headless player). `/ask` = resident-operator: call the configured model and optionally parse+apply its typed proposal batch. | `51`, `52`, `16`, `50`, `ADR-0001/0002`, `docs/self-heal.md` |
